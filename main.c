...
bit lock=1; KtLock=0;// trang thai khoa
...
void bip()
{
    ...
    {
		buzz=0; delay_us(30);
        buzz=1; delay_us(30); // 100us = 10khz
    }
...
void CaiThoiGian()
{
	...
	if(MODE==4)
	{
		...
		while(MODE==4)
		{
			if(TimeOut==1) {TimeOut=0; goto exit;}
			LCD_Gotoxy(0,0);
			...
			else if(mp==12) {LCD_Clear();MODE++;dem1=dem2=0; TR0=1;bip();}	//phim Menu
...
void HenGioTB1()
{
	...
	{ 	
		...
		while(MODE==2)
		{
			...
			else if(mp==12) {MODE++;bip();dem1=dem2=0; TR0=1;}//phim Menu		   {LCD_Clear();MODE++;dem1=dem2=0; TR0=1;bip();}
...
void HenGioTB2()
{
	...
	{	
		...
		while(MODE==3)
		{
			...
			else if(mp==12) {MODE++; bip();dem1=dem2=0; TR0=1;}//phim Menu
...
void KiemTraThoiGianTuDong()
{
	if((gioontb1!= gioofftb1) || (phutontb1!=phutofftb1))
	{
		...
		{
			TB1=0;	bip(); delay_ms(1000);
		}
		if(trangthaicaidatofftb1==1 && gio==gioofftb1 && phut==phutofftb1 && giay<2) 
		{
			TB1=1; bip(); delay_ms(100); bip(); delay_ms(1000);
		}
		delay_ms(1000);
	}
	if((gioontb2!= gioofftb2) || (phutontb2!=phutofftb2))
	{
		...
		{
			TB2=0;	bip(); delay_ms(1000);
		}
		if(trangthaicaidatofftb2==1 && gio==gioofftb2 && phut==phutofftb2 && giay<4) 
		{
			TB2=1; bip(); delay_ms(100); bip(); delay_ms(1000);
		}
	}
}
...
void NhapMatKhauDTMF()
{
	...
	while(i<4)
	{
		if(TimeOut==1) {Cancel=1;TimeOut=0; break;}
		if(get_key()==14) {Cancel=1;break;}
		mp = GiaiMaCodeDTMF();
		if(mp<10)
		{
			if(i!=3) bip();
...
{	
	...
	while(DTMF==1)
	{
		...
		if(TimeOut==1) {TimeOut=0; goto thoat;} 
		mp = GiaiMaCodeDTMF();
		if(mp==1)//so 1
		{
			tb1=1;
...
{
	bip(); delay_ms(100); bip(); delay_ms(100); bip();
	DTMF = 1;
	while(STD==0);
	MODE=0;	
	dem1=dem2=0;
